...
// Generate slug unik dalam lingkup kategori tertentu.
// excludeId dipakai saat update, supaya lokasi itu sendiri tidak dianggap bentrok.
async function generateSlugUnik(kategoriId, nama, excludeId = null) {
  const base = buatSlug(nama);
  let slug = base;
  let counter = 1;
...
export const createLokasi = async (data) => {
  ...
  // 3. Generate slug unik dalam kategori ini
  const slug = await generateSlugUnik(catData.id, data.nama);

  // 4. Insert ke tabel lokasi utama
  ...
  if (error) throw error;
  return newLokasi; // Mengembalikan data berisi 'id' dan 'slug'
};
...
// =====================================================================
// LOKASI — UPDATE
// Catatan: slug SENGAJA tidak diregenerasi otomatis di sini supaya link
// yang sudah dibagikan/terindeks (SEO) tidak putus saat admin ganti nama.
// Kalau memang perlu ganti slug, sediakan field slug terpisah di form admin
// dan panggil generateSlugUnik(kategoriId, namaBaru, id) secara eksplisit.
// =====================================================================
...
// Panggil fungsi ini secara terpisah dari form admin kalau admin memang
// ingin sengaja mengganti slug sebuah lokasi (jarang dipakai).
export const updateSlugLokasi = async (id, kategoriId, namaBaru) => {
  const slugBaru = await generateSlugUnik(kategoriId, namaBaru, id);
  const { error } = await supabase.from("lokasi").update({ slug: slugBaru }).eq("id", id);
  if (error) throw error;
  return slugBaru;
};
...
export const uploadFotoLokasi = async (lokasiId, file, isUtama, urutan) => {
  ...
  const { data: publicUrlData } = supabase.storage.from("galeri_desa").getPublicUrl(filePath);

  const { error: dbError } = await supabase.from("lokasi_foto").insert({
    lokasi_id: lokasiId,
...
export const setFotoUtamaLokasi = async (lokasiId, fotoId) => {
  ...
  const { error: err2 } = await supabase.from("lokasi_foto").update({ is_utama: true }).eq("id", fotoId);
  if (err2) throw err2;
};
...
export const replaceProdukLokasi = async (lokasiId, produkValid) => {
  const { error: delErr } = await supabase.from("lokasi_produk").delete().eq("lokasi_id", lokasiId);
...
function formatJamOperasionalList(jamList) {
  ...
  return jamList
    .map((jo) => {
      const rangeHari = jo.hari_mulai === jo.hari_selesai ? jo.hari_mulai : `${jo.hari_mulai}-${jo.hari_selesai}`;
...
export const getLokasiPeta = async () => {
  ...
  return (data || [])
    ...
    .map((row) => {
      ...
      const hargaValid = (row.lokasi_produk || []).map((p) => p.harga).filter((h) => h != null);
      ...
      return {
        ...
        slug: row.slug,
        nama: row.nama,
        kategori: kategoriSlug, // dipertahankan: dipakai untuk filter kategori di peta
        kategoriSlug,
        lat: row.latitude,
        lng: row.longitude,
...
// =====================================================================
// HALAMAN DETAIL POTENSI PUBLIK (/potensi/:kategoriSlug/:lokasiSlug)
// =====================================================================
export const getLokasiDetailPublik = async (kategoriSlug, lokasiSlug) => {
  ...
  // Cegah akses lewat kombinasi kategori yang salah, mis. /potensi/wisata/pak-rahmad
  // padahal pak-rahmad sebenarnya kategori bengkel.
  if (row.kategori_potensi?.slug !== kategoriSlug) return null;
  ...
  return {
    id: row.id,
    slug: row.slug,
    kategoriSlug: row.kategori_potensi?.slug,
    nama: row.nama,
...
export const getLokasiTerdekat = async (excludeId) => {
  ...
  return (data || [])
    ...
    .map((row) => {
      const fotoUtama =
        row.lokasi_foto?.find((f) => f.is_utama) || row.lokasi_foto?.slice().sort((a, b) => a.urutan - b.urutan)[0];
...
export const getSemuaLokasiKategori = async () => {
  const { data, error } = await supabase.from("v_lokasi_dengan_foto_utama").select("*").order("nama");
...
export const getKategoriPotensi = async () => {
  const { data, error } = await supabase.from("kategori_potensi").select("id, nama, slug").order("nama");
  if (error) throw error;
  return data;
};
...
export const getDusunList = async () => {
  const { data, error } = await supabase.from("dusun").select("id, nama").order("nama");
  if (error) throw error;
  return data;
};
...
export const getStatistikPotensi = async () => {
  const { data, error } = await supabase.from("lokasi").select("kategori_potensi ( nama )").eq("status", "aktif");

  if (error) throw error;

  const rows = data || [];
  const hitung = (namaList) => rows.filter((r) => namaList.includes(r.kategori_potensi?.nama)).length;
...
export const getStatistikDashboard = async () => {
  const { data, error } = await supabase.from("lokasi").select("id, kategori_potensi ( nama )").eq("status", "aktif");
...
export const getDistribusiDusun = async () => {
  const { data, error } = await supabase.from("lokasi").select("id, dusun ( nama )").eq("status", "aktif");
