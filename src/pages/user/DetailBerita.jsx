...
import { FaChevronLeft, FaCalendarAlt, FaWhatsapp } from "react-icons/fa";
import { getBeritaById, getAllBerita } from "../../services/beritaService"; // sesuaikan path
...
const WARNA_BADGE_KATEGORI = {
  ...
  Wisata: "bg-purple-50 text-purple-600",
};
...
function DetailBerita() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setError(null);
      try {
        const hasil = await getBeritaById(id);
        if (!mounted) return;
        ...
        const fotoUtama =
          hasil.berita_foto?.find((f) => f.is_utama)?.url ||
          hasil.berita_foto?.[0]?.url ||
          FALLBACK_IMG;
        ...
        // Berita lainnya, kecualikan berita ini sendiri
  ...
  return (
    <div className="min-h-screen bg-[#F5FBF1] pb-16">
      <div className="max-w-6xl mx-auto px-6 pt-6">
        ...
        <div className="relative rounded-2xl overflow-hidden bg-gray-100 h-56 sm:h-80">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6 mt-6">
          <div className="flex flex-col gap-4 min-w-0">
            ...
            {/* isi berupa HTML dari rich text editor */}
            <div
                  ...
                  <div className="p-3">
                    <span
                      className={`text-[10px] font-semibold uppercase rounded-full px-2 py-0.5 ${
                        WARNA_BADGE_KATEGORI[b.kategori_nama] || "bg-green-50 text-green-700"
                      }`}
                    >
                      {b.kategori_nama}
                    </span>
